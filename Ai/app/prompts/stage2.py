...
6-1. book, music 도메인이 exclude_domains에 없으면 반드시 각 4개를 채워라. 빈 배열 반환 절대 금지.
...
11. year은 개봉 연도를 작성한다.
12. author, artist 는 반드시 원어로 작성한다. ("제인오스틴" ->"Jane Austen" )
13. original_title은 원제(원어 제목)로 표기하라. 한국어 번역 제목이 아닌 영어·일어 등 원어 기준으로 작성한다. (예: "로마의 휴일" → "Roman Holiday", "센과 치히로의 행방불명" → "千と千尋の神隠し")
...
- history에 포함된 콘텐츠는 추천 결과에 절대 포함하지 마라.
- history의 각 노드 title과 동일하거나 동일 작품의 다른 표기(영문/한글)인 경우도 제외한다.
- exclude_domains에 없는 도메인은 반드시 4개를 채운다. 빈 배열([])로 반환하지 않는다.

[출력 스키마]
{
  "recommendations": {
    "movie": [{"title": "","original_title":"","year":"", "reason": "", "tags": [""], "connection_keyword": ""},    ],
